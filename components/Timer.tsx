...
const Timer: React.FC = () => {
  ...
  // Get Baghdad/Duhok time (Arabian Standard Time - AST)
  const baghdadTime = new Date(time.toLocaleString("en-US", {timeZone: "Asia/Baghdad"}));
  
  const formattedTime = baghdadTime.toLocaleTimeString('en-US', {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
    hour12: true
  ...
  const period = baghdadTime.getHours() >= 12 ? 'PM' : 'AM';
  ...
  return (
    <div className="glass-card rounded-3xl p-6 text-center shadow-xl border-amber-500/20 mb-8 relative overflow-hidden group">
      ...
      <h3 className="text-amber-400 font-medium mb-2 tracking-wide uppercase text-sm">دەمی نوکە - بەغداد و دهۆک ({period})</h3>
